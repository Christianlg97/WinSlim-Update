...
namespace wumgr
{
    static class Program
    {
        ...
        public static string mName = "WinSlim Update";
        private static string nTaskName = "WinSlimUpdateNoUAC";
        ...
        static void Main(string[] args)
        {
            ...
            mVersion = fvi.FileMajorPart + "." + fvi.FileMinorPart + "." + fvi.FileBuildPart;

            wrkPath = appPath;

            Translate.Load(IniReadValue("Options", "Lang", "es"));

            AppLog Log = new AppLog();
            AppLog.Line("{0}, versión {1}. Basado en WuMgr de David Xanatos", mName, mVersion);
            AppLog.Line("Software libre distribuido bajo GNU General Public License, versión 3.\r\n");

            string pipeName = TestArg("-preview")
                ? "wumgr_preview_" + Process.GetCurrentProcess().Id
                : "wumgr_pipe";
            ipc = new PipeIPC(pipeName);

            var client = ipc.Connect(100);
            if (client != null)
            {
                AppLog.Line("La aplicación ya se está ejecutando.");
                client.Send("show");
            ...
            if (!MiscFunc.IsAdministrator() && !MiscFunc.IsDebugging() && !TestArg("-preview"))
            {
                ...
                if (!MiscFunc.IsRunningAsUwp())
                {
                    ...
                    catch
                    {
                        ...
                        AppLog.Line("Se necesitan permisos de administrador para instalar actualizaciones.");
                    }
                }
            }
            ...
            AppLog.Line("Carpeta de trabajo: {0}", wrkPath);
        ...
        static public bool SkipUacEnable(bool is_enable)
        {
            ...
            {
                AppLog.Line("No se pudo habilitar el inicio sin aviso UAC: {0}", err.ToString());
                return false;
            }
            return true;
        }
        ...
        static public bool SkipUacRun()
        {
            ...
            try
            {
                ...
                silent = false;
                AppLog.Line("Intentando iniciar mediante la tarea autorizada...");
            ...
            {
                if(!silent)
                    AppLog.Line("Error al iniciar mediante la tarea autorizada: {0}", err.ToString());
            }
            return false;
        }
