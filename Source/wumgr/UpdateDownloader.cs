...
namespace wumgr
{
    class UpdateDownloader
    {
        ...
        {
            if (Progress != null)
                Progress(this, new WuAgent.ProgressArgs(mDownloads.Count, mDownloads.Count == 0 ? 0 : (100 * mCurrentTask + args.Percent) / mDownloads.Count, mCurrentTask + 1, args.Percent, mInfo));
        }
        ...
        void OnFinished(object sender, HttpTask.FinishedEventArgs args)
        {
            if (!args.Cancelled)
            {
                ...
                if (!args.Success)
                {
                    AppLog.Line("La descarga ha fallado: {0}", args.GetError());
                    if (mCurTask.DlName != null && File.Exists(mCurTask.DlPath + @"\" + mCurTask.DlName))
                        AppLog.Line("Se usará la versión anterior que ya está descargada.");
                    else
